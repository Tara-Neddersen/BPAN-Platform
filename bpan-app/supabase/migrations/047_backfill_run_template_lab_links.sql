-- ============================================================
-- BPAN Platform — Backfill sync for run/template/lab-aware flows
-- Idempotent and safe to re-run. Supports dry-run reporting.
-- ============================================================

alter table public.datasets
  add column if not exists experiment_run_id uuid references public.experiment_runs(id) on delete set null;

create index if not exists datasets_experiment_run_id_idx
  on public.datasets (experiment_run_id);
...
revoke all on function public.backfill_platform_sync(boolean, integer) from public;
grant execute on function public.backfill_platform_sync(boolean, integer) to service_role;
